...
| Option | Description |
|--------|-------------|
| `--watch` | Auto-start execution when a new actionable task appears while the instance is stopped (not paused). |
| `--poll <seconds>` | Re-fetch the task list every N seconds (1–3600). `0` disables polling. Equivalent to pressing `r` in the TUI. |

Both are CLI-only flags. They do not appear in the in-TUI settings menu
and are not persisted to `.ralph-tui/config.toml` — pass them on every
invocation when you want continuous-mode behavior.
...
Keep Ralph running 24/7 against an active tracker and let it pick up new
tasks automatically:

```bash
# Re-check the tracker every 30s; auto-start when a new task appears.
ralph-tui run --watch --poll 30 --epic prod-backlog
```

`--poll` triggers the same refresh as pressing `r` in the TUI. `--watch`
fires the same start-or-continue logic as pressing `s` — but only when the
engine is in a stopped-but-not-paused state (`stopped`, `idle`, `complete`)
and the refresh revealed a new actionable task. Both flags are CLI-only:
they do not appear in the in-TUI settings menu and are not persisted to
`.ralph-tui/config.toml`. Wrap your launch in a shell alias or a
systemd/launchd unit if you want the same flags on every run.

<Callout type="info">
Auto-start does not fire while Ralph is paused, executing, or in an
`error` state. It is also suppressed for remote tabs — auto-driving
execution across machines without explicit per-instance control is
